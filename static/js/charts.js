...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var otuIDs = first_result.otu_ids;
    var otuLabs = first_result.otu_labels;
    var sampleVals = first_result.sample_values;
    ...
    var trace_bar = {
      x: (Object.values((sampleVals.slice(0,10)).sort((a,b)=> a-b))
                          .map(num => parseInt(num))),
      y: (Object.values((otuIDs.slice(0,10)).reverse())
                          .map(str => String('OTU ' + str))),
      mode: 'markers',
      type: 'bar',
      name: 'bacterium IDs',
      text: ((otuLabs.slice(0,10)).sort((a,b)=> a-b)),
      orientation:'h'
    };
